...
export default function MemberProfile() {
  ...
  const gameHistory = useMemo(() => {
    ...
    return Data.data.filter((item) => item.status === "end");
  }, [Data?.data]);

  const hasGameData = Data?.data?.length;

  const orientation =
    hasGameData && Data.data[0]?.white_username === username
      ? "white"
      : "black";

  const memberRequestFormTempObj = hasGameData
    ? generateMemberRequestFormObj(Data.data[0])
    : null;
              ...
              <div className="sm:ml-6 mt-4 sm:mt-0 text-center sm:text-left">
                <div className="flex items-baseline gap-3">
                  ...
                  <span className="text-sm text-gray-400 font-normal">
                    member since{" "}
                    {orientation === "white"
                      ? new Date(Data.data[0].white_created_at).toDateString()
                      : new Date(Data.data[0].black_created_at).toDateString()}
                  </span>
                </div>
                ...
                <div className="mt-4 flex space-x-2 justify-center sm:justify-start">
                  <button
                    onClick={() => {
                      PlayContext.setMemberRequestForm({
                        ...PlayContext.memberRequestForm,
                        memberData: memberRequestFormTempObj,
                      });
                      navigate(`/member/challenge/${username}`);
                    }}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  <div className="text-2xl font-bold text-gray-600">
                    {Data?.data?.length && !Data?.data[0]?.isNoGameData
                      ? Data?.data?.length
                      : "new account"}
                  </div>
                      ...
                      >
                        <div>
                          <p className="font-semibold">
                            ...
                            <NavLink to={`/game/${game.id}`}>↗️</NavLink>
                            <span
                              className={`font-bold ${
